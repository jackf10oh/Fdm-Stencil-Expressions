...
class SparseDiag : public Eigen::SparseMatrixBase<SparseDiag<ArgType> > {
  public:
    ...
    // constructors 
    SparseDiag(const ArgType& arg) : m_arg(arg) {
      ...
      if(arg.rows()!=1) throw std::invalid_argument("SparseDiag must be constructed from MatrixStorage_t."); 
    }
    
    // member functions 
    Index rows() const { return m_arg.cols(); }
...
struct evaluator<SparseDiag<ArgType> > : evaluator_base<SparseDiag<ArgType> > {
  ...
  struct InnerIterator{
      ...
      : m_eval(eval), m_row(row_idx)
    {
      ...
      m_valid = (m_row < m_eval.rows()) && (m_eval.m_argImpl.coeff(0,m_row) != Scalar(0.0)); 
    };
    // member data  
    ...
    Scalar value() const { return m_eval.m_argImpl.coeff(0,m_row); };
    // member data
