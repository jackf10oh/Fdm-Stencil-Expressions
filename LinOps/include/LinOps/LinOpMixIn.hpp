...
class LinOpMixIn
{
  ...
  public:
    // Type Defs -------------------------------------
    typedef struct{} is_linop_tag; // to tell if a class derived from LinOpBase1D<> or XD<> 
    ...
    {
      return static_cast<typename LINOPBASE_T::DERIVED_T*>(this)->GetMat(); 
    };
    decltype(auto) GetMat() const
    {
      return static_cast<const typename LINOPBASE_T::DERIVED_T*>(this)->GetMat(); 
    };
    ...
    auto compose(DerivedInner&& InnerOp) &
    {
      ...
      return compose_impl<DerivedInner,typename LINOPBASE_T::DERIVED_T&>(std::forward<DerivedInner>(InnerOp)); 
  ...
  private:
    ...
    auto left_scalar_mult_impl(SCALAR_T&& c) & {
      return LinOpExpr<SCALAR_T, typename LINOPBASE_T::DERIVED_T&, internal::scalar_left_mult_op>(
        std::forward<SCALAR_T>(c), // lhs scalar
        static_cast<typename LINOPBASE_T::DERIVED_T&>(*this), // rhs 
  ...
  public:
    ...
    auto operator+(LINOP_T&& rhs) & {
        return LinOpExpr<typename LINOPBASE_T::DERIVED_T&, LINOP_T, internal::linop_bin_add_op>(
        static_cast<typename LINOPBASE_T::DERIVED_T&>(*this),  // lhs
    ...
    template<typename LINOP_T, typename = std::enable_if_t<traits::is_linop_crtp<LINOP_T>::value>>
    auto operator-(LINOP_T&& rhs) & {
        return LinOpExpr<typename LINOPBASE_T::DERIVED_T&, LINOP_T, internal::linop_bin_subtract_op>(
        static_cast<typename LINOPBASE_T::DERIVED_T&>(*this), //lhs
        std::forward<LINOP_T>(rhs), //rhs
    ...
    // unary operator-() (lval) ---------------------------------------------- 
    auto operator-() & {
        return LinOpExpr<typename LINOPBASE_T::DERIVED_T&, void, internal::unary_negate_op>(
        static_cast<typename LINOPBASE_T::DERIVED_T&>(*this), // lhs 
