...
class DiffOpExpr : public DiffOpBase<DiffOpExpr<Lhs_t, Rhs_t, BinaryOp_t, ORDER>, ORDER>
{
  public:
    // Type Defs --------------------------------------
  ...
  public:
    ...
    DiffOpExpr()=delete; 
    // from Lhs + Rhs 
    DiffOpExpr(LStorage_t A, RStorage_t B)
      : m_Lhs(A), m_Rhs(B), m_BinOp() 
    { /* some types of static asserts might be needed here ...*/};
    // copy 
    DiffOpExpr(const DiffOpExpr& other)=default; 
    // destructor
    ~DiffOpExpr()=default;
    ...
    {
      return m_BinOp(m_Lhs,m_Rhs,weights, n_nodes_per_row, ith_node); 
    } 
...
template<typename Lhs_t, typename UnaryOp_t, std::size_t ORDER>
class DiffOpExpr<Lhs_t, void, UnaryOp_t, ORDER> : public DiffOpBase<DiffOpExpr<Lhs_t, void, UnaryOp_t, ORDER>, ORDER>
{
  public:
    // Type Defs --------------------------------------
    using LStorage_t = typename traits::Storage_t<Lhs_t>::type;
  ...
  public:
    ...
    double CoeffAt(const Cont& weights, std::size_t n_nodes_per_row, std::size_t ith_node) const 
    {
      return m_UnarOp(m_Lhs, weights, n_nodes_per_row, ith_node); 
    } 
