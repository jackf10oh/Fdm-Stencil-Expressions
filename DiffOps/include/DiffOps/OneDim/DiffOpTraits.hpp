...
template<typename Lhs_t, typename Rhs_t, typename BinaryOp_t, std::size_t ORDER>
class DiffOpExpr; 
...
{
  template<typename L, typename R, typename Cont>
  double operator()(const L& lhs, const R& rhs, const Cont& weights, std::size_t n_nodes_per_row, std::size_t ith_node) const 
  {
    return lhs.CoeffAt(weights, n_nodes_per_row, ith_node) + rhs.CoeffAt(weights, n_nodes_per_row, ith_node); 
  } 
}; 
...
struct diffop_left_mult_op
{
  ...
  double operator()(const L& lhs, const R& rhs, const Cont& weights, std::size_t n_nodes_per_row, std::size_t ith_node) const 
  {
    return lhs * rhs.CoeffAt(weights, n_nodes_per_row, ith_node); 
  } 
}; 
...
{
  template<typename L, typename Cont>
  double operator()(const L& lhs, const Cont& weights, std::size_t n_nodes_per_row, std::size_t ith_node) const 
  {
    return -lhs.CoeffAt(weights, n_nodes_per_row, ith_node);  
  } 
}; 
...
template<typename L, typename R, typename OP, std::size_t ORDER>
struct is_diffop_expr_crtp_impl<DiffOpExpr<L,R,OP,ORDER>>: public std::true_type
{};
} // end namespace internal 
