...
int main()
{
  ...
  my_vals.set_init(my_mesh, bump_1d); 
  ...
  // foo bar ing ------------------------------------------------------------------ 
  my_interp.FillVals(100); 

  // std::size_t N = 10; 
  // for(std::size_t idx = 0; idx < time_mesh->size(); idx += time_mesh->size() / N)
  // {
  //   double t = time_mesh->at(idx); 
  //   cout << "solution at " << t << ":  "; 
  //   print_vec((*my_interp.m_data)[idx]); 
  // }
  ...
  std::cout << "solution a t: ["; 
  auto it = my_mesh->cbegin();
  auto end = std::prev(my_mesh->cend());  
  for(; it!=end; it++)
  {
    std::cout << my_interp.at(t, std::vector<double>(1,*it)) << ", "; 
  }
  std::cout << my_interp.at(t, {*it}) << "]" << std::endl; 
};